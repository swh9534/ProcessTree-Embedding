...
for idx, LOG_FILENAME in enumerate(LOG_FILENAMES):
    ...
    for i in range(len(intervals)-1):
        ...
        for noise_threshold in noise_thresholds:
            ...
            filename = f"all_BPIC_ver3/{idx}_process_tree_case_size_betwee_{lower_bound}_and_{upper_bound}_noise_{int(noise_threshold*100)}.svg"
            serialization = tree.to_string()
            if serialization in serializations:
                continue
            serializations.add(serialization)
            # append to process_trees.txt
            with open(f"all_BPIC_ver3/0_metadata.txt", "a") as file:
        ...
        # 메타데이터 파일에 추가
        metadata_filename = f"all_BPIC_ver3/80_interval5/{LOG_FILENAME}/{i+1}_metadata.txt"
